#!/usr/bin/env python3
"""
FIX-10: Independent Production model.nano Verifier
====================================================
Parses raw bytes of model.nano WITHOUT importing export_to_nano.py
or export_production_nano.py. Completely standalone.

Usage:
  python tools/verify_production_model_nano.py --nano models/model.nano
"""
import os, sys, json, zlib, struct, hashlib, argparse, datetime
...
EXPECTED_FP32_BYTES      = 1_320_960
EXPECTED_INT8_BYTES      = 335_544_320
EXPECTED_TERNARY_BYTES   = 428_605_440
...
def parse_header(data):
    assert len(data)>=HEADER_SIZE, f"File too small for header"
...
def parse_descriptors(data):
    ...
    for i in range(TENSOR_COUNT):
        ...
        raw=data[off:off+DESCRIPTOR_SIZE]
...
def verify(nano_path):
    ...
    print("="*80)
    print(f"FIX-10 INDEPENDENT BINARY VERIFIER | {ts}")
    ...
    with open(nano_path,"rb") as f:
        data=f.read()
    assert len(data)==EXPECTED_FINAL_SIZE
    ...
    # 4. Header size (we verify by checking payload starts at expected offset)
    ...
    # 17. Quantization types: only FP32=0, INT8=2, TERNARY=4
    ...
    # 18. Scales (FP32 scale=1.0, INT8/TERNARY scale>0)
    for d in descs:
        if d["qt"]==NANO_QUANT_FP32: assert abs(d["scale"]-1.0)<1e-5, f"FP32 scale {d['scale']} != 1.0 for id={d['id']}"
    ...
    total_numel=0
    for d in descs:
        total_numel+=numel_from_quant(d["qt"],d["size_bytes"])
    ...
    # 23. CRC (independent)
    raw_after_header=data[HEADER_SIZE:]
    crc_computed=zlib.crc32(raw_after_header)&0xFFFFFFFF
    ...
    print("MODEL_NANO_SHA256="+final_sha)
