...
authoritative production teacher (Qwen/Qwen2.5-7B-Instruct) and 2.05B student:
  ...
  3. Executes 10 real training steps (forward, soft teacher logits, loss, backward, optimizer step)
  4. Records step number, loss, GPU allocated/reserved/peak, nonzero gradient tensors, parameter delta per step
  5. Saves checkpoint to Google Drive / output directory
  6. Reloads checkpoint into fresh model and verifies numerical consistency
...
def run_real_10_step_smoke_test(
    ...
    # Snapshot initial state for total 10-step delta tracking
    ...
    for step in range(1, total_steps + 1):
        ...
        # Step G: Measure Parameter Delta for this Step
        step_l1_delta = 0.0
        step_changed_tensors = 0
        for name, p in student.named_parameters():
            if p.requires_grad:
                diff = (p.detach().cpu().float() - step_pre_weights[name].float()).abs().sum().item()
                step_l1_delta += diff
                if diff > 0.0:
                    step_changed_tensors += 1

        print(f"  Step {step:2d}/10 | Loss: {loss_val:.6f} | Grads: {nonzero_grads:3d}/{student_trainable_tensors} | Changed: {step_changed_tensors:3d} | L1 Delta: {step_l1_delta:10.2f} | VRAM: {alloc_mb:.0f}/{resv_mb:.0f} MB (Peak: {max_alloc_mb:.0f} MB) | Time: {t_elapsed:.2f}s")

        step_records.append({
            "step": step,
            "loss": loss_val,
            "nonzero_grads": nonzero_grads,
            "changed_tensors": step_changed_tensors,
            "step_l1_delta": step_l1_delta,
    ...
    # Total 10-step cumulative parameter delta
    total_10step_delta = 0.0
    total_changed_tensors = 0
    for name, p in student.named_parameters():
        if p.requires_grad:
            diff = (p.detach().cpu().float() - initial_all_weights[name].float()).abs().sum().item()
            total_10step_delta += diff
            if diff > 0.0:
                total_changed_tensors += 1

    print("\n" + "=" * 80)
    print("10-STEP CUMULATIVE NUMERICAL AUDIT")
    print("=" * 80)
    print(f"TOTAL_10_STEP_L1_DELTA:   {total_10step_delta:.4f}")
    print(f"TOTAL_CHANGED_TENSORS:    {total_changed_tensors} / {student_trainable_tensors}")
    ...
    if total_10step_delta <= 0.0:
        print("[FATAL ERROR] Model parameters did not change across 10 steps!")
        print("REAL_10_STEP_TRAINING_FAIL: Zero cumulative parameter delta.")
        return 1

    # 5. Save Smoke Checkpoint
    ...
    print("[Checkpoint] Reloading checkpoint into fresh model to verify state...")
